...
const DeleteModal = ({ title, description, ButtonAction, isDialogOpen, setIsDialogOpen }: Props) => {
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <DialogContent className="!w-[320px] !outline-none !border-none bg-black-100 select-none p-8 rounded-xl">
        <section className="flex flex-col space-y-7"> {/* Body */}
          <DialogHeader className="flex flex-col space-y-2">
            <DialogTitle className="font-extrabold tracking-wide text-xl">{title}</DialogTitle>
